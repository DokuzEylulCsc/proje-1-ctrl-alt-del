    ...
    class Oyuncu
    {
        ...
        public void KelimeUygunluk(string GirilenKelime )
        {
            ...
            else// uyusmuyorsa
            {
                ...
                string[][] harfler; // AI yanlis dogru ve dopdogru olanlari sirasi ile dondurucek methoda sahip olucak
                                  // 0. indis yanlislar 1.indis dogrular 2.indis dopdogrular

                for(int i=0; i < harfler[0].Length; i++)
                {
                    if(harfler[0][i]== "ç")
                    {
                        harfler[0][i] = "c1";
                    ...
                    f2.Controls[harfler[0][i]].BackColor = Color.Red;
                }       // Yanlis kelimeler kontrolu
                for (int i = 0; i < harfler[1].Length; i++)
                {
                    if (harfler[1][i] == "ç")
                    {
                        harfler[1][i] = "c1";
                    ...
                    f2.Controls[harfler[1][i]].BackColor = Color.Yellow;
                }   // Dogru kelimeler kontrolu
                for (int i = 0; i < harfler[2].Length; i++)
                {
                    if (harfler[2][i] == "ç")
                    {
                        harfler[2][i] = "c1";
                    ...
                    f2.Controls[harfler[2][i]].BackColor = Color.Green;
                }   // Dogru yer dogru kelimeler kontrolu
                ((ListBox)f2.Controls["listBox12"]).Items.Add(girilen);
            }
        }
        public void PasGec()
        {
            ...
            if ( a > 0)
            {
                f2.Controls["tahmin"].Text = "";
                a--;
                f2.Controls["pas"].Text = a.ToString();
            }
            else
            {
                MessageBox.Show("Can Hakkin Yok");
            }
